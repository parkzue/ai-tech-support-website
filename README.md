<p>Website for OpenAI technical support agent.</p>

<p>To host this website on cloud we are using Render</p>
<p>https://render.com/</p>

<p>This project has two parts: ai-tech-support and ai-tech-support-website. The former is for creating the AI tech support logic and the latter is the website for accessing and using the AI tech support chatbots.</p>
<p>https://github.com/parkzue/ai-tech-support</p>

<p>Here is a screenshot of the chat window</p>
<img width="100%" src="https://github.com/parkzue/ai-tech-support-website/blob/main/src/images/chatwindow.png?raw=true"></a>
